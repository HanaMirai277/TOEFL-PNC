        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ms-auto mx-lg-auto mb-2 mb-lg-0 text-end text-lg-start">
                ...
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('jadwal') ? 'active' : '' }}"
                        href="{{ route('jadwal') }}">Jadwal Tes</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{ request()->routeIs('hasiltes') ? 'active' : '' }}"
                        href="{{ route('hasiltes') }}">Hasil Tes</a>
                </li>
            </ul>
            <div class="d-flex align-items-center justify-content-end justify-content-lg-start">
                ...
                @else
                    @php
                        $authUser = Auth::user();
                        $avatarUrl = $authUser?->foto_profil
                            ? asset('storage/' . $authUser->foto_profil)
                            : asset('images/profil2.jpg');
                    @endphp
                    <div class="user-avatar-container" id="userAvatarToggle">
                        <img src="{{ $avatarUrl }}" alt="{{ $authUser?->name }}" class="user-avatar">
                        <div class="user-dropdown" id="userDropdown">
                            <div class="px-3 py-2">
                                <p class="mb-0 fw-bold text-dark" style="font-size: 0.9rem;">{{ $authUser?->name }}</p>
                            ...
                            <form action="{{ route('logout') }}" method="POST">
                                @csrf
                                <button type="submit"
                                    class="dropdown-item-custom w-100 border-0 bg-transparent text-danger">
                                    <img src="{{ asset('icons/logout.png') }}" alt=""
                                        style="filter: invert(30%) sepia(100%) saturate(2000%) hue-rotate(340deg) brightness(90%) contrast(100%);">
                                    <span>Logout</span>
                                </button>
                            </form>
                        </div>
