                    ...
                    <div class="row g-4">
                        <div class="col-lg-6">
                            <div class="card border-0 shadow-sm h-100" style="border-radius: 20px;">
                                <div class="card-header section-header" style="border-top-left-radius: 20px; border-top-right-radius: 20px;">
                                    Informasi Jadwal Tes
                                </div>
                                <div class="card-body p-4">
                                    <div class="detail-row">
                                        <span>Nama Tes</span><strong>{{ $pendaftaranTes->jadwalTes->judul_tes }}</strong>
                                    </div>

                                    <div class="detail-row">
                                        <span>Jenis Tes</span><strong>{{ $pendaftaranTes->jadwalTes->jenis_tes }}</strong>
                                    </div>

                                    <div class="detail-row">
                                        <span>Tanggal Tes</span><strong>{{ tanggal_panjang($pendaftaranTes->jadwalTes->tanggal_tes) }}</strong>
                                    </div>
                        ...
                        <div class="col-lg-6">
                            <div class="card border-0 shadow-sm h-100" style="border-radius: 20px;">
                                ...
                                <div class="card-body p-4">
                                    <div class="detail-row">
                                        <span>Nama Lengkap</span><strong>{{ $pendaftaranTes->nama_peserta }}</strong>
                                    </div>

                                    <div class="detail-row">
                                        <span>Jenis Kelamin</span><strong>{{ $pendaftaranTes->jenis_kelamin }}</strong>
                                    </div>

                                    <div class="detail-row">
                                        <span>Status</span><strong class="text-capitalize">{{ $pendaftaranTes->status_pendaftar }}</strong>
                                    </div>

                                    @if ($pendaftaranTes->nim)
                                        <div class="detail-row">
                                            <span>NIM</span><strong>{{ $pendaftaranTes->nim }}</strong>
                                        </div>
                                    @endif

                                    @if ($pendaftaranTes->program_studi)
                                        <div class="detail-row">
                                            <span>Program Studi</span><strong>{{ $pendaftaranTes->program_studi }}</strong>
                                        </div>
                                    @endif

                                    @if ($pendaftaranTes->tahun_lulus)
                                        <div class="detail-row"><span>Tahun Lulus</span><strong>{{ $pendaftaranTes->tahun_lulus }}</strong>
                                        </div>
                                    @endif

                                    @if ($pendaftaranTes->no_ktp)
                                        <div class="detail-row">
                                            <span>Nomor KTP</span><strong>{{ $pendaftaranTes->no_ktp }}</strong>
                                        </div>
                                    @endif

                                    <div class="detail-row">
                                        <span>Nomor WhatsApp</span><strong>{{ $pendaftaranTes->no_wa }}</strong>
                                    </div>
                                    ...
                                    <div class="detail-row mb-0">
                                        <span>Keperluan Tes</span><strong>{{ $pendaftaranTes->keperluan_tes }}</strong>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-6">
                            <div class="card border-0 shadow-sm h-100" style="border-radius: 20px;">
                                ...
                                <div class="card-body p-4 d-flex flex-column align-items-center">
                                    ...
                                    <div class="text-muted mt-3">Pembayaran akan dilakukan melalui QRIS pada langkah selanjutnya.</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-end gap-3 mt-5">
                            <a href="{{ route('pendaftaran.step1', $pendaftaranTes) }}" class="btn btn-outline-custom py-3">
                                Ubah Data
                            </a>
                            <form action="{{ route('pendaftaran.konfirmasi', $pendaftaranTes) }}" method="POST">
                                @csrf
                                <button type="submit" class="btn btn-simpan py-3">Konfirmasi dan Lanjutkan Pembayaran</button>
                            </form>
                        </div>
                    </div>
                </div>
