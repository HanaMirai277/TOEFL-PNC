...
class HasilTesController extends Controller
{
    public function index()
    {
        ...
        // Ambil semua data pendaftaran tes beserta data hasilnya untuk user yang sedang login
        ...
        // Filter hanya pendaftaran yang sudah memiliki data hasil tes
        $hasilList = $pendaftaranList
            ->filter(fn ($p) => $p->hasilTes !== null)
            ->map(fn ($p) => $this->formatHasil($p))
            ->values();

        // Ambil hasil terbaru/utama (diurutkan berdasarkan jadwal_tes tanggal_tes)
    ...
    public function unduhSuratPengambilanPdf(Request $request): Response
    {
        ...
        $statusPeserta      = $pendaftaran->status_pendaftar
            ? ucfirst(strtolower($pendaftaran->status_pendaftar)) : null;
    ...
    /**
     * Format data pendaftaran dan hasil tes untuk ditampilkan ke view
     */
